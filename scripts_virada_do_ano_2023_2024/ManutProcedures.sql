...
BEGIN
    ...
    -- Atualiza��o de dados modificados no SGP
    update destino
     ...
     -- Inativa��o dos registros que n�o est�o mais vindo do SGP
     update destino
       set State = 3,
           UpdateDate = GETDATE()
      from CentralProjects..AdministrativeUnit destino
...
BEGIN
	...
	-- Cadastra as permiss�es para os docentes
    ...
    BEGIN
       ...
       -- Manuten��o da tabela SSIS_LoginImportado
    ...
    BEGIN
       ...
       -- UsuarioGrupo
       -- Deleta a rela��o UsuarioGrupo para os registros que est�o na tmp de servidores importados automaticamente
       DELETE ug
         FROM CoreSSO..SYS_UsuarioGrupo ug
              INNER JOIN SSIS_LoginImportado lo
               ON ug.usu_id = lo.usu_id
    ...
    begin
       ...
       -- Manuten��o da tabela SSIS_LoginCoordPedag
       MERGE SSIS_LoginImportado AS _target
       USING (SELECT usu.usu_id, usu.usu_login, tmp.gru_id
...
AS
BEGIN
	-- Apaga os dados da �ltima integra��o (precisa apagar todos por causa da integridade referencial)
...
/*    ------------------------------- IN�CIO do tratamento de v�nculo indevido -------------------------------
    ...
    BEGIN
        ...
        -- Exclui logicamente a movimenta��o de entrada do v�nculo indevido, desde que n�o esteja em uma  
        -- movimenta��o de sa�da de deva permanecer
        update entrada
        ...
        -- Exclui logicamente a movimenta��o de sa�da do v�nculo indevido, desde que n�o esteja em uma  
        -- movimenta��o de entrada de deva permanecer
        update saida
           set mov_situacao = 3, mov_dataAlteracao = GETDATE(), usu_id = '8D869FB9-F88C-E311-B1FE-782BCB3D2D76'
        ...
        -- Atualiza a movimenta��o de sa�da que esteja ligada a uma entrada com v�nculo indevido
        update saida
        ...
        if @@ERROR <> 0
        begin
           print 'Erro na atualiza��o do registro indevido na MTR_Movimentacao (sa�da)'
           rollback
           return
        end
        
        -- Atualiza a movimenta��o de entrada que esteja ligada a uma sa�da com v�nculo indevido
        update entrada
        ...
        if @@ERROR <> 0
        begin
           print 'Erro na atualiza��o do registro indevido na MTR_Movimentacao (entrada)'
           rollback
           return
        end
        
        -- Exclui logicamente os alunos que s� tinham a movimenta��o de v�nculo indevido
        update alu
           set alu_situacao = 4
        ...
        if @@ERROR <> 0
        begin
           print 'Erro na atualiza��o de alunos'
           rollback
           return
        end
    ...
    drop table #Arrumar
    ------------------------------ T�RMINO do tratamento de v�nculo indevido ------------------------------- */

    IF (@ent_id IS NULL)
      SET @ent_id = (SELECT ent_id FROM SSO_SYS_Entidade WITH (NOLOCK) WHERE ent_sigla = 'smesp')
    
    /* cria tabela Tempor�ria com a �ltima movimenta��o do aluno */
    ...
    -- Pega a �ltima movimenta��o e os respectivos ids
...
BEGIN
    ...
    /* Limpa a tabela tempor�ria para gravar a movimenta��o dos novos alunos */
...
/* TRATAMENTO EXCLUSIVO PARA OS MTDS DAS EXPERI�NCIAS PEDAG�GICAS DE TERRIT�RIOS DO SABER */
	...
	--inserindo primeiro os MTDS para os ttes que est�o sendo incluidos nesta execu��o
	...
	select alu_id, mtu_id, maxMtd_id + linha as mtd_id, tud_id, mtd_dataMatricula, mtu_numeroChamada as mtd_numeroChamada,
	  ...
	  from (
			select MT.alu_id, MT.mtu_id, MT.tud_id, mtu_dataMatricula as mtd_dataMatricula, --� ISSO MESMO. MTU_DATAMATRICULA POIS PRECISA TER A MESMA DATA DE MATR�CULA INDEPENDENTE PRA TODOS OS MTDS INDEPENDENTE DE QUANDO COME�OU O TTE
	...
	--			  A N�O SER QUE O ALUNO TENHA SA�DO DA TURMA (mtu_situacao = 5 que � o update de cima)
    ...
    -- Inativa as matr�culas de Recupera��o Paralela e AEE que n�o est�o mais vindo na view
    update mtd
       SET mtd_situacao = 5,
    ...
    -- Inativa as matr�culas de Educa��o f�sica em turmas espec�ficas j� encerradas
    update mtd
    ...
    -- Exclui logicamente as matr�culas de Educa��o F�sica que estiverem inativas
    update mtd
       set mtd_situacao = 3, mtd_dataAlteracao = GETDATE()
      from GE_MTR_MatriculaTurmaDisciplina mtd
           inner join GE_MTR_MatriculaTurma mtu with (nolock)
...
BEGIN
	...
	SELECT tmp.alu_id, tmp.mtu_id, tmp.alc_id, tmo.tmv_idEntrada, tmo.tmv_idSaida,
     ...
     WHERE tmp.cl_st in ('I','S','E')
       ...
       and tur.cal_id = turAnt.cal_id
       and mtr.cur_id <> mtrAnt.cur_id 
	   and mtr.cur_id = cr.cur_id and mtrAnt.cur_id = cr.cur_idRelacionado -- essa � a diferen�a p parte de cima do UNION, q leva em conta os relacionados
       and mtr.crr_id = mtrAnt.crr_id
       and mtr.crp_id <> mtrAnt.crp_id
       and tmp.alu_id = mtr.alu_id
       AND tmp.mtu_id = mtr.mtu_id
    ...
    ----------------- Arruma n�mero de chamada para as matr�culas ativas (Educa��o F�sica) -----------------
    ...
    -------------------------FIM ATUALIZACAO do n�mero de chamada (Educa��o F�sica)-------------------------
	   ...
	   from (select mtu.alu_id, mtu.mtu_id 
			   from GestaoPedagogica..ACA_AlunoCurriculo alc
				    ...
				    inner join GestaoPedagogica..MTR_MatriculasBoletim mb 
						   ...
						   and mb.mtu_id is not null
					INNER JOIN GestaoPedagogica..MTR_MatriculaTurma mtu2 on mtu2.alu_id = alc.alu_id and mtu2.mtu_id = mb.mtu_id
																			  and mtu2.cur_id not in (253,254) --cursos sem TUD E MTD
			  where alc.alc_situacao <> 3
				and mtu.mtu_situacao <> 3 --and alc.esc_id between 500 and 600
			    and not exists (select mb.alu_id from GestaoPedagogica..MTR_MatriculasBoletimDisciplina mb 
								 where mb.alu_id = mtu.alu_id
	 ...
	 EntradaTransfOutrasRedes, mtu_origemDados 
		from (   select mtd.alu_id, mb.cal_ano, mb.cal_id, mb.cap_id, mb.tpc_id, mb.tpc_ordem, mtd.mtu_id, 
				  ...
				  where alc_situacao <> 3
					...
					and mtd_situacao <> 3
					and tud_situacao <> 3
					and tud.tud_tipo <> 10 -- Recupera��o paralela n�o funcionaria pois utilizam o mesmo tds_id
...
BEGIN
	...
	*/
	...
	ON (_source.pes_id = _target.pes_id
		...
		AND _source.psd_numero = _target.psd_numero)
	WHEN MATCHED THEN
		UPDATE SET
...
BEGIN
	...
	(
		SELECT tmp.cd_cargo, tmp.dc_cargo, tvi.tvi_id
		 ...
		 --adicionando essa parte de baixo do union, para colocar os cargos de professores que est�o com lota��es nulas
		 --se faz necess�rio por causa da parte de atribui��o espor�dica
		 SELECT tmp.cd_cargo, tmp.dc_cargo, tvi.tvi_id
...
BEGIN
	...
	SELECT tua_id FROM CoreSSO..SYS_TipoUnidadeAdministrativa
	 WHERE tua_nome in ('Diretoria Regional de Educa��o','Escola')
...
DECLARE
	...
	-- Id do sistema de Manuten��o
		SELECT  @sis_idManutencao = sis_id
		FROM    SSO_SYS_Sistema
		WHERE   sis_nome = 'Manuten��o'

	-- ID do Grupo Manuten��o
		SELECT @gru_idManutencao = gru_idUsadoIntegracao FROM DEPARA_GRUPOS_INTEGRACAO
        WHERE nomeUsadoIntegracao = 'Grupo Manuten��o' and sis_id = @sis_idManutencao
		
	-- Insere na tabela tempor�ria os usu�rios que n�o poder�o ser exclu�dos, pois esses n�o foram inclu�dos pela importa��o.
...
BEGIN
	...
	SET @gru_id_diretor_escolar = (SELECT gru_idUsadoIntegracao FROM DEPARA_GRUPOS_INTEGRACAO 
									WHERE nomeUsadoIntegracao = 'diretores escolares' and sis_id = 144)
	SET @tua_id_dre = (SELECT tua_id FROM SSO_SYS_TipoUnidadeAdministrativa WHERE LOWER(tua_nome) = 'diretoria regional de educa��o')
	...
	--> SYS_UsuarioGrupoUA
	 -- Apenas inser��o pois o Delete j� � feito acima, garantindo assim qualquer atualiza��o.	
...
BEGIN
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela ACA_CalendarioAnual'
       ROLLBACK
       RETURN
    END
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela ACA_TipoTurno'
       ROLLBACK
       RETURN
    END
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela SYS_UnidadeAdministrativa'
       ROLLBACK
       RETURN
    END
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela ESC_Escola'
       ROLLBACK
       RETURN
    END
	...
	WITH CteDestinationTurma AS (
		...
		WHERE tur_id IN ( 
			...
			--where c.tme_id NOT IN (4,6,7,8) Retirando exclus�o de turmas EJA para que entrem no serap
		)
	)
    ...
    USING (select tur.tur_id, tur.esc_id, tur.tur_codigo, tur.tur_descricao, tur.cal_id, trn.ttn_id,
             ...
             from GestaoPedagogica..TUR_Turma tur with (nolock)
                  ...
                  on tur.cal_id = cal.cal_id
				  INNER JOIN GestaoPedagogica..TUR_TurmaCurriculo tc ON tc.tur_id = tur.tur_id
			      INNER JOIN GestaoAvaliacao_SGP..ACA_Curso c ON c.cur_id = tc.cur_id
            where tur.tur_situacao <> 3
			  ...
			  and tur_tipo IN (1,6)
			 -- and c.tme_id NOT IN (4,6,7,8)  Retirando exclus�o de turmas EJA para que entrem no serap
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela TUR_Turma'
       ROLLBACK
       RETURN
    END
	...
	--ele fica com a altera��o 2 e n�o cai mais na clausula Destino.tur_situacao not in (2,3) do merge 
	UPDATE turSerap 
	...
	(
		...
		--WHERE aluno.alu_id NOT IN ( SELECT alu_id FROM #CteIdDosAlunosDoEjaeCieja )  Retirando exclus�o de turmas EJA para que entrem no serap
	)
    ...
    USING (select alu.alu_id, alu.pes_id, pes.pes_nome, alu.ent_id, alc.alc_matricula,
			...
			--  and alu.alu_id NOT IN ( SELECT alu_id FROM #CteIdDosAlunosDoEjaeCieja ) Retirando exclus�o de turmas EJA para que entrem no serap
    ...
    WHEN MATCHED
         AND ((Destino.alu_nome COLLATE DATABASE_DEFAULT <> Origem.pes_nome COLLATE DATABASE_DEFAULT)
			  ...
			  OR
			  (Destino.alu_matricula <> Origem.alc_matricula))
         THEN
         UPDATE SET alu_nome = Origem.pes_nome,
					alu_matricula = Origem.alc_matricula,
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela ACA_Aluno'
       ROLLBACK
       RETURN
    END
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela ACA_TipoModalidadeEnsino'
       ROLLBACK
       RETURN
    END
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela ACA_TipoNivelEnsino'
       ROLLBACK
       RETURN
    END
    ...
    WHEN NOT MATCHED BY SOURCE AND Destino.tcp_situacao <> 3  --AND tme_id NOT IN (4,6,7,8) 
	    THEN
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela ACA_TipoCurriculoPeriodo'
       ROLLBACK
       RETURN
    END
    ...
    WHEN NOT MATCHED BY SOURCE AND Destino.cur_situacao <> 3 -- AND tme_id NOT IN (4,6,7,8) Para entrar alunos eja 
	 THEN
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela ACA_Curso'
       ROLLBACK
       RETURN
    END
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela ACA_Curriculo'
       ROLLBACK
       RETURN
    END
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela ACA_CurriculoPeriodo'
       ROLLBACK
       RETURN
    END
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela ACA_TipoDisciplina'
       ROLLBACK
       RETURN
    END
    ...
    USING (select crd.cur_id, crd.crr_id, crd.crp_id, tds.tds_id, crd.crd_situacao, min(crd.crd_tipo) as crd_tipo,
                  crp.tcp_id, max(crd.crd_dataCriacao) as crd_dataCriacao, max(crd.crd_dataAlteracao) as crd_dataAlteracao
            ...
            group by crd.cur_id, crd.crr_id, crd.crp_id, tds.tds_id, crp.tcp_id, crd.crd_situacao) Origem
     ON Destino.cur_id = Origem.cur_id
    AND Destino.crr_id = Origem.crr_id
    AND Destino.crp_id = Origem.crp_id
    ...
    WHEN MATCHED
         ...
         UPDATE SET crd_tipo = Origem.crd_tipo,
					...
					tcp_id = Origem.tcp_id,
                    crd_dataAlteracao = Origem.crd_dataAlteracao
    WHEN NOT MATCHED THEN
         INSERT (cur_id, crr_id, crp_id, tds_id, crd_tipo, tcp_id, crd_situacao, crd_dataCriacao, crd_dataAlteracao)
         VALUES (Origem.cur_id, Origem.crr_id, Origem.crp_id, Origem.tds_id, Origem.crd_tipo,  Origem.tcp_id,
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela ACA_CurriculoDisciplina'
       ROLLBACK
       RETURN
    END
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela TUR_TurmaCurriculo'
       ROLLBACK
       RETURN
    END
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela TUR_TurmaDisciplina'
       ROLLBACK
       RETURN
    END
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela MTR_MatriculaTurma'
       ROLLBACK
       RETURN
    END
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela MTR_MatriculaTurmaDisciplina'
       ROLLBACK
       RETURN
    END
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela ACA_Docente'
       ROLLBACK
       RETURN
    END
	...
	USING (SELECT t.tur_id, cur.cur_id, cur.tme_id, cur.tne_id, crp.crp_ordem, t.esc_id, cur.cur_situacao, t.tur_situacao, crp.crp_situacao, tcr.tcr_situacao
		...
		where t.tur_situacao = 1 --AND cur.tme_id --NOT IN (4,6,7,8)
    ...
    IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na integra��o de dados na tabela TUR_TurmaDocente'
       ROLLBACK
       RETURN
    END
	...
	--- Inativa as atribui��es de docentes para que n�o acessem as turmas de CIEJA e/ou EJA modular no SGP ----
	update GestaoPedagogica..TUR_TurmaDocente
	  ...
	  from GestaoPedagogica..TUR_TurmaDocente
		   ...
		   inner join GestaoPedagogica..ACA_Curso curEJA 
				   ...
				   --and cur_nome_abreviado in ('ENS MED TC', 'ENS MED MG','CIEJA', 'EJA MOD') --aguardando a publica��o do EJA do SGP
				   and cur_nome_abreviado in ('ENS MED TC', 'ENS MED MG', 'CIEJA', 'EJA MOD', 'EJA', 'EJA ESP') --aguardando a publica��o do EJA
       ...
       and not exists (select tcr2.tur_id
						 from GestaoPedagogica..TUR_TurmaCurriculo tcr2
							   inner join GestaoPedagogica..ACA_Curso cur2 
									  ...
									  --and cur2.cur_nome_abreviado in ('CIEJA', 'EJA MOD') --aguardando a publica��o do EJA do SGP
									  and cur2.cur_nome_abreviado in ('CIEJA', 'EJA MOD', 'EJA', 'EJA ESP') --aguardando a publica��o do EJA do SGP
	...
	update GestaoPedagogica..ACA_CurriculoEscola set ces_situacao = 3, ces_dataAlteracao = GETDATE() where cur_id in (253,254) and ces_situacao <> 3
	...
	SELECT @gru_idSupervisor = gru.gru_idUsadoIntegracao FROM DEPARA_GRUPOS_INTEGRACAO gru
	 WHERE gru.nomeUsadoIntegracao = 'Supervis�o Escolar' and gru.sis_id = @sis_id
    ...
    BEGIN
        ...
        SELECT usu.usu_id, serv.gru_id, uad.uad_id
          FROM (SELECT rf, cd_escola, @gru_idProfessor AS gru_id
                          ...
                          from (select prf.rf, gc.cd_escola
                                  from BD_PRODAM..v_cadastro_professor prf
									   ...
									   inner join DEPARA_SERIE dep on dep.cd_serie_ensino = tm.cd_serie_ensino
											  and ((dep.cur_id in (245,247,248) and dep.crp_ordem > 2) --EF (menos o de 4 horas) a partir do terceiro ano (a pedido do Hygor por email em 05/05/2016)
												    or (dep.cur_id in (39,40,41,42,249,250,251) and dep.crp_ordem in (2,4,6,8))) --EJA apenas algumas s�ries (a pedido do Hygor por email em 05/05/2016)
                                       inner join BD_PRODAM..v_unidade_educacao_dados_gerais esc
                                       on gc.cd_escola = esc.cd_unidade_educacao
                                 where esc. dc_tipo_unidade_educacao = 'ESCOLA' 
                                   and esc.sg_tp_escola in ('EMEF','EMEFM','CEU EMEF','EMEBS','CIEJA',
                ...
                SELECT ds.cd_registro_funcional AS rf, cb.lotacao, @gru_idProfessor AS gru_id
				 ...
				 --Servidores que possuem cargo base de diretor ou coordenador pedag�gio e n�o possuem cargo sobreposto
					...
					WHERE 
						...
						AND crg.cd_cargo IN (3360, 3379, 3085, 3352) -- Diretor, Coordenador, 3182 Secret�rio, 3085 Assistente de Diretor, 3352  Supervisor
				...
				--Servidores que passaram a ser coordenadores pedag�gicos ou diretores atrav�s do cargo sobreposto
					...
					WHERE 
						crs.cd_cargo IN (3360, 3379, 3085, 3352) -- Diretor, Coordenador, 3182 Secret�rio, 3085 Assistente de Diretor, 3352  Supervisor
		...
		----Remove os UsarioGrupo que n�o possuem mais acesso
		DELETE ug
			FROM CoreSSO..SYS_UsuarioGrupo ug
            WHERE 
        ...
        -- UsuarioGrupoUA
        -- Apaga a rela��o UsuarioGrupoUA para os registros que est�o na tmp de servidores importados automaticamente
        DELETE ugu
          FROM CoreSSO..SYS_UsuarioGrupoUA ugu
               INNER JOIN SSIS_LoginImportado lo
		...
		--Remove os UsarioGrupoUA que n�o possuem mais acesso
		DELETE ug
        ...
        WHEN NOT MATCHED THEN
	   	 ...
	   	 -- CORRIGE UAD_CODIGO PARA USUARIOS DAs DRES FINAL 02 DEVIDO CADASTRO INCORRETO NO EOL

	   -- 16/03/2024  **

		DECLARE @uad_id      UNIQUEIDENTIFIER,
		        @gru_id      UNIQUEIDENTIFIER,
		        @novo_uad_id UNIQUEIDENTIFIER,
		        @uad_codigo  VARCHAR(10),
		        @tua_id      UNIQUEIDENTIFIER;
		
		SELECT @tua_id = tua_id
		FROM   coresso..sys_tipounidadeadministrativa
		WHERE  tua_nome = 'Diretoria Regional de Educa��o';
		
		DECLARE cursor_dados CURSOR FOR
		  SELECT DISTINCT a.uad_id,
		                  a.primeiros_digitos + '00' uad_codigo,
		                  a.gru_id
		  FROM   (SELECT ua.uad_id,
		                 ua.uad_nome,
		                 ua.uad_codigo,
		                 Substring(ua.uad_codigo, 0, Len(ua.uad_codigo) - 1)
		                 primeiros_digitos,
		                 Substring(ua.uad_codigo, Len(ua.uad_codigo) - 1,
		                 Len(ua.uad_codigo))
		                    dois_ultimos_digitos
		                        ,
		                 uga.gru_id
		          FROM   coresso..sys_usuariogrupoua uga
		                 INNER JOIN coresso..sys_unidadeadministrativa ua
		                         ON uga.uad_id = ua.uad_id
		                 INNER JOIN coresso..sys_grupo g
		                         ON uga.gru_id = g.gru_id
		          WHERE  ua.tua_id = @tua_id -- DRE
		                 AND g.sis_id = 204) a
		  WHERE  a.dois_ultimos_digitos = '02'
		
		OPEN cursor_dados;
		
		FETCH next FROM cursor_dados INTO @uad_id, @uad_codigo, @gru_id;
		
		WHILE @@FETCH_STATUS = 0
		  BEGIN
		      SELECT @novo_uad_id = ua.uad_id
		      FROM   coresso..sys_unidadeadministrativa ua
		      WHERE  ua.uad_codigo = @uad_codigo
		
		      UPDATE CORESSO..sys_usuariogrupoua
		      SET    uad_id = @novo_uad_id
		      WHERE  gru_id = @gru_id
		             AND uad_id = @uad_id
...
BEGIN
    ...
    UPDATE tmp
      ...
      FROM dbo.tmp_DiarioSupervisor_servidor tmp
           INNER JOIN SSO_SYS_Usuario u ON tmp.cd_registro_funcional = u.usu_login
           INNER JOIN SSO_PES_Pessoa p ON u.pes_id = p.pes_id
     WHERE tmp.usu_id is null --depois faz pras outras situa��es, pra quando n�o existir um usuario ativo
       AND u.usu_situacao <> 3
       AND p.pes_situacao <> 3
    ...
    IF @gru_id IS NOT NULL
	   -- Desabilita todos registros vinculados aos usu�rios vindos da importa��o do grupo Individual
       UPDATE ug
          SET usg_situacao = 3
         FROM SSO_SYS_UsuarioGrupo ug
    ...
    IF @gru_idPub IS NOT NULL
	   -- Desabilita todos registros vinculados aos usu�rios vindos da importa��o do grupo P�blico
       UPDATE ug
          SET usg_situacao = 3
         FROM SSO_SYS_UsuarioGrupo ug
              INNER JOIN dbo.SSIS_SMESP_Controle_Grupo_Individual cont ON ug.usu_id = cont.usu_id
	...
	BEGIN
       ...
       -- Atualiza ativos vindos da importa��o e ativos do Grupos Individual
       UPDATE ug
          SET usg_situacao = 1
         FROM SSO_SYS_UsuarioGrupo ug
	...
	BEGIN
       ...
       -- Atualiza ativos vindos da importa��o e ativos do Grupo P�blico
       UPDATE ug
          SET usg_situacao = 1
         FROM SSO_SYS_UsuarioGrupo ug
              INNER JOIN dbo.SSIS_SMESP_Controle_Grupo_Individual cont
    ...
    BEGIN
	   ...
	   -- Desabilita todos registros vinculados aos usu�rios vindos da importa��o do grupo Sala de Leitura - UA
       UPDATE ug
          SET usg_situacao = 3
         FROM SSO_SYS_UsuarioGrupo ug
	   ...
	   /**************************************************************************************/
       /*                                                                                    */
       /* Colocar na tabela tmp_DRE_SalaLeitura as outras DREs conforme solicita��o do Hygor */
       /*                                                                                    */
       /**************************************************************************************/
       ...
       -- Atualiza ativos vindos da importa��o e ativos
       UPDATE ug
          SET usg_situacao = 1
         FROM SSO_SYS_UsuarioGrupo ug
       ...
       -- Apaga todos os registros na UsuarioGrupoUA para os usu�rios vindos da importa��o
     ...
     SELECT @trn_id = trn_id, @ent_id = ent_id, @ttn_id = ttn_id, @trn_descricao = trn_descricao,
	  ...
	  BEGIN
			...
			SELECT 
				...
				@IntervaloDiferente = CASE WHEN qtdInvervaloPeriodo > 0 THEN 2 ELSE 0 END, --posi��o do intervalo diferente
...
BEGIN
 -- Teste CAique
	...
	select tg.*
	 ...
	 where cd_etapa_ensino in (1,2,3,4,5,6,9,10,11,13,14,17)
		
	--Altera��o do c�digo do curso de ensino infantil especial para regular
	...
	-- em reuni�o no dia 04/01/2017 o Valmir nos falou que esta diferencia��o tem rela��o com a ordem de matr�cula de alunos e portanto n�o importa pro SGP
...
UNION
-- o select abaixo � espec�fico pra disciplinas de infantil porque as disciplinas precisam ter diferencia��es (TDS_IDS diferentes) do ber�ario/minigrupo
SELECT distinct cd_componente_curricular, dc_componente_curricular,
...
union
-- o select abaixo � espec�fico pra disciplinas de ber�ario e minigrupo porque as disciplinas precisam ter diferencia��es (TDS_IDS diferentes) do infantil
SELECT distinct cd_componente_curricular, dc_componente_curricular,
...
UPDATE DEPARA_DISCIPLINAS 
   SET dis_nome = ACA_TipoDisciplina.tds_nome
  FROM DEPARA_DISCIPLINAS , GestaoPedagogica..ACA_TipoDisciplina 
 WHERE tne_id = 2 and tds_nome = 'Educa��o f�sica'
...
UPDATE DEPARA_DISCIPLINAS 
   SET dis_nome = ACA_TipoDisciplina.tds_nome
  FROM DEPARA_DISCIPLINAS , GestaoPedagogica..ACA_TipoDisciplina 
 WHERE tne_id = 1 and tds_nome = 'Conceito Global (Ber�ario/Minigrupo)' and tds_nome = dis_nome
...
UPDATE DEPARA_DISCIPLINAS 
 ...
 WHERE tne_id = 2 and tds_nome = 'L�ngua portuguesa' 
   ...
   -- verificar se tem diferen�a da 1098 do regular pro especial
...
UPDATE DEPARA_DISCIPLINAS 
 ...
 select dis_codigo, dis_nome, tds_nome, 0 as disciplinaDeficiente, 0 as disciplinaDuplaRegencia
  ...
  where dis_codigo <> 0
    ...
    and tds_nome like 'Recupera%' --aguardando publica��o
...
FROM    ( SELECT    cd_serie_ensino ,
        ...
        INNER JOIN GE_ACA_Curso cur WITH ( NOLOCK ) 
				ON cur.cur_nome COLLATE DATABASE_DEFAULT = _out.crr_nome COLLATE DATABASE_DEFAULT
				   or (cur.cur_id = 239 and _out.crr_nome = 'Ensino Fundamental Especial de 9 Anos')
				   or (cur.cur_id = 241 and _out.crr_nome = 'Ensino M�dio Regular')
...
GROUP BY 
	...
	UPDATE #curCurriculoPeriodo SET cur_id = 251 where cur_id = 41
	...
	SELECT crp.crp_ordem, CAST(crp.crp_ordem as varchar(1))+'� ano', 5, crr_nome, 2, 5, 
	 ...
	 where div.cur_id not in (239,241,242,249,250,251,41,42,253,254)
	...
	delete from #curCurriculoPeriodo where cur_id in (235,236,237,238)
	
	INSERT INTO #curCurriculoPeriodo
	values (1, 'Fundamental I Libras', 13, 'Ensino Fundamental 9 Anos',                  2, 5, 'CICLO I',  0, 1, 242, 1, 1, 1, 0, 226)
	    ,(1, 'Fundamental I Libras', 13, 'Ensino Fundamental 9 Anos',                    2, 5, 'CICLO I',  0, 1, 243, 1, 1, 1, 0, 226)
		,(1, '1� EJA Modular'      ,  3, 'Educa��o de Jovens e Adultos (EJA - Modular)', 2, 5, 'CICLO I',  0, 1,  42,  1, 4, 1, 0, 212)
		,(2, '2� EJA Modular'      ,  3, 'Educa��o de Jovens e Adultos (EJA - Modular)', 2, 5, 'CICLO I',  0, 1,  42,  1, 4, 2, 0, 213)
		,(3, '3� EJA Modular'      ,  3, 'Educa��o de Jovens e Adultos (EJA - Modular)', 2, 5, 'CICLO II', 0, 1,  42,  1, 4, 3, 0, 210)
		,(4, '4� EJA Modular'      ,  3, 'Educa��o de Jovens e Adultos (EJA - Modular)', 2, 5, 'CICLO II', 0, 1,  42,  1, 4, 4, 1, 211)	
		
		,(1, 'EJA ALFABETIZACAO I' ,  3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 249, 1, 8, 1, 0, 274)
		,(2, 'EJA ALFABETIZACAO II',  3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 249, 1, 8, 2, 0, 275)	
		,(3, 'EJA BASICA I',		  3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 249, 1, 8, 3, 0, 278)	
		,(4, 'EJA BASICA II',		  3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 249, 1, 8, 4, 0, 280)	
		,(5, 'EJA COMPLEMENTAR I',    3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 249, 1, 8, 5, 0, 246)	
		,(6, 'EJA COMPLEMENTAR II',   3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 249, 1, 8, 6, 0, 247)	
		,(7, 'EJA FINAL I',			  3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 249, 1, 8, 7, 0, 248)	
		,(8, 'EJA FINAL II',		  3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 249, 1, 8, 8, 1, 249)	
		                                                                                                          
		,(1, 'EJA ALFABETIZACAO I',   3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 249, 1, 8, 1, 0, 276)
		,(2, 'EJA ALFABETIZACAO II',  3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 249, 1, 8, 2, 0, 277)	
		,(3, 'EJA BASICA I',		  3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 249, 1, 8, 3, 0, 279)	
		,(4, 'EJA BASICA II',		  3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 249, 1, 8, 4, 0, 281)	
		,(5, 'EJA COMPLEMENTAR I',    3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 249, 1, 8, 5, 0, 265)	
		,(6, 'EJA COMPLEMENTAR II',   3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 249, 1, 8, 6, 0, 266)	
		,(7, 'EJA FINAL I',			  3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 249, 1, 8, 7, 0, 267)	
		,(8, 'EJA FINAL II',		  3, 'EJA - Educa��o de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 249, 1, 8, 8, 1, 268)	
		                                                                                
		,(3, 'EJA BASICA I',		  11, 'EJA - Educa��o de Jovens e Adultos',          2, 5, 'CICLO I',  0, 1, 250, 1, 8, 3, 0, 292)	
		,(4, 'EJA BASICA II',		  11, 'EJA - Educa��o de Jovens e Adultos',          2, 5, 'CICLO I',  0, 1, 250, 1, 8, 4, 0, 294)	
		,(4, 'EJA BASICA II',		  11, 'EJA - Educa��o de Jovens e Adultos',          2, 5, 'CICLO I',  0, 1, 250, 1, 8, 4, 0, 295)	
		,(5, 'EJA COMPLEMENTAR I',    11, 'EJA - Educa��o de Jovens e Adultos',          2, 5, 'CICLO II', 0, 1, 250, 1, 8, 5, 0, 261)	
		,(6, 'EJA COMPLEMENTAR II',   11, 'EJA - Educa��o de Jovens e Adultos',          2, 5, 'CICLO II', 0, 1, 250, 1, 8, 6, 0, 270)	
		,(6, 'EJA COMPLEMENTAR II',   11, 'EJA - Educa��o de Jovens e Adultos',          2, 5, 'CICLO II', 0, 1, 250, 1, 8, 6, 0, 262)	
		,(7, 'EJA FINAL I',			  11, 'EJA - Educa��o de Jovens e Adultos',          2, 5, 'CICLO II', 0, 1, 250, 1, 8, 7, 0, 263)	
		,(8, 'EJA FINAL II',		  11, 'EJA - Educa��o de Jovens e Adultos',          2, 5, 'CICLO II', 0, 1, 250, 1, 8, 8, 1, 272)	
		
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 235, 1, 6, 5, 0, 23)	
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 235, 1, 6, 5, 0, 24)	
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 235, 1, 6, 5, 0, 225)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 235, 1, 6, 6, 1, 118)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 235, 1, 6, 6, 1, 25)	
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 235, 1, 6, 6, 1, 26)	
		                                                                                 
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 236, 1, 6, 5, 0, 23)	
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 236, 1, 6, 5, 0, 24)	
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 236, 1, 6, 5, 0, 225)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 236, 1, 6, 6, 1, 118)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 236, 1, 6, 6, 1, 25)	
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 236, 1, 6, 6, 1, 26)	
		                                                                                 
		,(4, 'Mini grupo II',          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 237, 1, 6, 4, 0, 28)	
		,(4, 'Mini grupo II',          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 237, 1, 6, 4, 0, 29)
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 237, 1, 6, 5, 0, 23)	
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 237, 1, 6, 5, 0, 24)
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 237, 1, 6, 5, 0, 225)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 237, 1, 6, 6, 1, 118)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 237, 1, 6, 6, 1, 25)	
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 237, 1, 6, 6, 1, 26)	
		                                                                                 
		,(1, 'Ber�ario I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 1, 0, 1)
		,(1, 'Ber�ario I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 1, 0, 3)
		,(2, 'Ber�ario II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 2, 0, 4)
		,(2, 'Ber�ario II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 2, 0, 6)
		,(3, 'Mini grupo I' ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 3, 0, 11)
		,(3, 'Mini grupo I' ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 3, 0, 27)
		,(4, 'Mini grupo II',          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 4, 0, 28)
		,(4, 'Mini grupo II',          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 4, 0, 29)
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 5, 0, 23)	
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 5, 0, 24)
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 5, 0, 225)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 6, 1, 118)		
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 6, 1, 25)	
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 238, 1, 6, 6, 1, 26)
		
		,(1, '1� Modulo'   ,          14, 'T�cnico M�dio',                               2, 5, 'SEM CICLO', 0, 1, 253, 1, 3, 1, 0, 194)
		,(2, '2� Modulo'   ,          14, 'T�cnico M�dio',                               2, 5, 'SEM CICLO', 0, 1, 253, 1, 3, 2, 0, 196)
		,(3, '3� Modulo'   ,          14, 'T�cnico M�dio',                               2, 5, 'SEM CICLO', 0, 1, 253, 1, 3, 3, 1, 198)
                                                                                        
		,(1, '1� Modulo'   ,          14, 'T�cnico M�dio',                               2, 5, 'SEM CICLO', 0, 1, 253, 1, 3, 1, 0, 282)
		,(2, '2� Modulo'   ,          14, 'T�cnico M�dio',                               2, 5, 'SEM CICLO', 0, 1, 253, 1, 3, 2, 0, 284)
		,(3, '3� Modulo'   ,          14, 'T�cnico M�dio',                               2, 5, 'SEM CICLO', 0, 1, 253, 1, 3, 3, 1, 286)
		                                                                                
		,(1, '1� Modulo'   ,          14, 'T�cnico M�dio',                               2, 5, 'SEM CICLO', 0, 1, 253, 1, 3, 1, 0, 283)
		,(2, '2� Modulo'   ,          14, 'T�cnico M�dio',                               2, 5, 'SEM CICLO', 0, 1, 253, 1, 3, 2, 0, 285)
		,(3, '3� Modulo'   ,          14, 'T�cnico M�dio',                               2, 5, 'SEM CICLO', 0, 1, 253, 1, 3, 3, 1, 287)

		,(1, '1� Ano'   ,              9, 'Ensino M�dio Magist�rio',                     2, 5, 'SEM CICLO', 0, 1, 254, 1, 4, 1, 0, 101)
		,(2, '2� Ano'   ,              9, 'Ensino M�dio Magist�rio',                     2, 5, 'SEM CICLO', 0, 1, 254, 1, 4, 2, 0, 103)
		,(3, '3� Ano'   ,              9, 'Ensino M�dio Magist�rio',                     2, 5, 'SEM CICLO', 0, 1, 254, 1, 4, 3, 0, 105)
		,(4, '4� Ano'	,              9, 'Ensino M�dio Magist�rio',                     2, 5, 'SEM CICLO', 0, 1, 254, 1, 4, 4, 1, 107)	
	...
	FROM (SELECT distinct grade.*, gc.qt_aula_semanal, depSer.crp_id, depSer.cur_id, depDis.cd_componente_curricular, depDis.dis_nome
		    FROM tmp_grade_mstech grade 
			   ...
			   INNER JOIN DEPARA_SERIE depSer 
					   ON depSer.cd_serie_ensino = grade.cd_serie_ensino 
					  and depSer.cur_id in (246,239,241,242,249,250,251,41,42,238)
		 WHERE (dt_fim_validade is null or dt_fim_validade >= GETDATE())
	   ) a 
	...
	UPDATE DEPARA_CURSO_GRADES 
	   SET cur_id = 245, crr_id = 1, crp_id = dep.crp_id
	  FROM DEPARA_CURSO_GRADES
	...
	UPDATE DEPARA_CURSO_GRADES 
	   SET cur_id = CASE WHEN soma_horas > 40 and cd_etapa_ensino = 5 THEN 248
					 WHEN soma_horas between 32 and 40 and cd_etapa_ensino = 5 and dep.crp_ordem <= 5 THEN 247
					 WHEN soma_horas between 30 and 40 and cd_etapa_ensino = 5 and dep.crp_ordem >= 6 THEN 247
					 WHEN soma_horas < 32 and cd_etapa_ensino = 5 and dep.crp_ordem <= 5 THEN 246
					 WHEN soma_horas < 30 and cd_etapa_ensino = 5 and dep.crp_ordem >= 6 THEN 246
					 WHEN soma_horas >= 31 and cd_grade not in (3541,3542,4285) and cd_etapa_ensino = 13 and dep.crp_ordem <= 6 THEN 240
					 WHEN soma_horas >= 30 and cd_grade not in (3541,3542,4285) and cd_etapa_ensino = 13 and dep.crp_ordem >= 7 THEN 240
					 WHEN soma_horas <= 30 and cd_grade not in (3541,3542,4285) and cd_etapa_ensino = 13 and dep.crp_ordem <= 6 THEN 239
					 WHEN soma_horas <= 29 and cd_grade not in (3541,3542,4285) and cd_etapa_ensino = 13 and dep.crp_ordem >= 7 THEN 239
					 WHEN soma_horas >= 51 and cd_etapa_ensino = 1 THEN 194
					 WHEN soma_horas between 37 and 50 and cd_etapa_ensino = 1 THEN 237
					 WHEN soma_horas between 26 and 36 and cd_etapa_ensino = 1 THEN 236
					 WHEN soma_horas <= 25 and cd_etapa_ensino = 1 THEN 235
					 WHEN cd_etapa_ensino = 6 THEN 241
					 WHEN DEPARA_CURSO_GRADES.cd_serie_ensino = 226 and cd_grade not in (4285) THEN 242
					 WHEN DEPARA_CURSO_GRADES.cd_serie_ensino = 226 and cd_grade = 4285 THEN 243
				 END, 
	 ...
	 WHERE dep.cur_id in (select cur_id from DEPARA_CURSOS_DIVIDIDOS) 
	   ...
	   and dep.cur_id not in (249,250,251,41,42) -- j� n�o entrariam por causa da DEPARA_CURSOS_DIVIDIDOS mas s� pra garantir
	...
	UPDATE DEPARA_CURSO_GRADES 
	  ...
	  FROM DEPARA_CURSO_GRADES
		   INNER JOIN DEPARA_SERIE dep ON dep.cd_serie_ensino = DEPARA_CURSO_GRADES.cd_serie_ensino
	 WHERE dep.cur_id in (249,250,251,41,42,253,254)
	   and DEPARA_CURSO_GRADES.cur_id is null
	...
	UPDATE DEPARA_CURSO_GRADES 
	 ...
	 WHERE dep.cur_id in (246,247,248)   
     ...
     UPDATE DEPARA_CURSO_GRADES
		SET fl_edFisica = 0 
	  WHERE cur_id in (240,242,243,245,247,248,41,42,235,236,237,238,253,254)
	  
	 UPDATE DEPARA_CURSO_GRADES
		SET fl_edFisica = 0 
	  WHERE cur_id = 246 and crp_id < 6
...
BEGIN
	...
	(
	  ...
	  WHERE     uad.ent_id = @ent_id
				...
				AND sg_tp_escola IN ('EMEF','EMEFM','CEU EMEF','EMEBS','CIEJA',
									 ...
									 'CEU CEMEI', 'CEU AT COMPL') --Adicionado by Fernando em 2024
	  GROUP BY 	uad.ent_id ,
				uad.uad_id ,
				tua_id ,
				uad_codigo ,
	...
	----Importa ESC_EscolaClassificacao. Decidi n�o deletar caso a classifica��o suma
	...
	--IN�CIO PARTE QUE ATUALIZA ENDERE�OS NAS TABELAS DO GesTAO
...
BEGIN
	...
	-- Id do sistema de Manuten��o
	SELECT  @sis_idManutencao = sis_id FROM SSO_SYS_Sistema WHERE sis_nome = 'Manuten��o'
    ...
    SELECT pes_nome, pes_dataNascimento, pes_sexo, 1, GETDATE(), GETDATE(), 1
     ...
     where not exists (select pes.pes_id from SSO_PES_Pessoa pes where pes.pes_id = #servidor.pes_id)
	 		...
	 		-- Excluir Pes_PessoaDocumento Duplicado antes de inserir os novos. 
	Delete  from SSO_PES_PessoaDocumento Where pes_id in (
	select pd.pes_id from SSO_PES_PessoaDocumento  pd 
	inner join #pessoaDoc_Table pt on pt.pes_id = pd.pes_id and pt.pes_cpf = pd.psd_numero) 
	...
	-- Cadastra as permiss�es para os docentes que n�o est�o na v_cadastro_professor
    ...
    begin
       ...
       WHEN MATCHED THEN
		...
		--al�m destes usu�rios, tamb�m tive que adicionar em 08/07 os usu�rios com atribui��o espor�dica no grupo 'Docente' do SGP
    ...
    --adicionado conforme solicita��o em 14/05/2015 para deixar marcado integracao = 2 para todos os servidores
    ...
    INSERT INTO PackageTaskLog (PackageLogID, SourceName, SourceID, StartDateTime)
         VALUES (@PackageLogID, 'Remover Permiss�es s unidade adm e coloca email falso p email nulo', @SourceID, getdate())
...
BEGIN
	...
	(
		SELECT
			...
			, CASE WHEN tmp.carga_horaria < 24 THEN 1 --JB
				   ...
				   --TODO: Testar a diferen�a entre JBD e JEIF
			  END
			  AS hora_atividade
		FROM
			tmp_DiarioClasse_cadastro_professor tmp
	...
	ON (_source.crg_id = _target.crg_id AND 
		...
		--TODO: quando a hora_atividade estiver correta no BD_PRODAM (diferen�a entre JBD e JEIF) descomentar a linha acima
	WHEN NOT MATCHED THEN
		INSERT
		(
			  ent_id
...
BEGIN
    ...
    MERGE INTO GE_RHU_ColaboradorCargo _target
    USING (SELECT distinct x.coc_id, x.pes_id, x.col_id, x.crg_id, x.uad_id, x.ent_id, x.chr_id,
				x.coc_situacao, x.coc_matricula, x.coc_observacao, x.coc_vigenciaInicio
			FROM (SELECT DISTINCT t.*,
						ROW_NUMBER() OVER(PARTITION BY t.col_id, t.crg_id, t.coc_id ORDER BY t.coc_vigenciaInicio, t.pes_id DESC) AS linha
					FROM (SELECT distinct
							...
							FROM (SELECT distinct tmp.pes_id, tmp.col_id, tmp.crg_id, tmp.uad_id, tmp.ent_id, tmp.chr_id,
										...
										ROW_NUMBER() OVER(PARTITION BY tmp.col_id, tmp.crg_id ORDER BY coc.coc_dataCriacao DESC) AS rowNum
									...
									AND coc.coc_situacao <> 3
									LEFT JOIN @dre dre
									ON dre.cd_unidade_educacao = tmp.lotacao) _main) t) x
				WHERE x.linha = 1) AS _source	
     ON _source.col_id = _target.col_id
    AND _source.crg_id = _target.crg_id
    AND _source.coc_id = _target.coc_id
		  ...
		  from (
				select coc.col_id, @crgAtribuicaoEsporadica as crg_id, 
				  ...
				  from GE_RHU_ColaboradorCargo coc
					   ...
					   inner join ( select prof.cd_escola, prof.cd_cargo_base_servidor, prof.rf, prof.cd_cargo, cb.lotacao, uad.ent_id, uad.uad_id
									from BD_PRODAM..v_cadastro_professor prof 
									inner join BD_PRODAM..v_cargobase_mstech cb on prof.rf = cb.cd_registro_funcional 
									and prof.cd_cargo_base_servidor = cb.cd_cargo_base_servidor
									INNER JOIN CoreSSO..SYS_UnidadeAdministrativa uad ON uad.uad_codigo = prof.cd_escola )
							cb on cb.rf = coc.coc_matricula 
							and cb.ent_id = coc.ent_id
							and cb.uad_id = coc.uad_id
					   inner join GE_RHU_Cargo crgDoc on cb.cd_cargo = crgDoc.crg_codigo
					                                  and crgDoc.crg_situacao <> 3 and crgDoc.crg_cargoDocente = 1
				 where coc_situacao = 1-- and coc.coc_matricula = '5925550'
	...
	WHEN NOT MATCHED THEN
      ...
      --inserido em 08/07 para garantir que todo docente com atribui��o espor�dica permane�a no grupo de docentes
      --inseri aqui pela urg�ncia da situa��o e porque ainda n�o consegui encontrar o local que os retira deste grupo
	  INSERT INTO CoreSSO..SYS_USUARIOGRUPO (usu_id, gru_id, usg_situacao)
...
BEGIN
	...
	SELECT coc.col_id, coc.crg_id, coc.coc_id, tds.tds_id
	 ...
	 WHERE (crg.crg_codigo in (3875,3212,3239,3220,4650) and tds_nome = 'Conceito Global (Ber�ario/Minigrupo)'
...
BEGIN
      ...
      FROM (SELECT DEPARA_TURMA.tur_id, es.esc_id, uni.uni_id,
              ...
              FROM tmp_DiarioClasse_turma tur 
				   INNER JOIN GE_ACA_CalendarioAnual cal ON cal.cal_ano = tur.an_letivo and cal.cal_situacao = 1
														AND ((tur.dc_etapa_ensino like '%infantil%' and cal.cal_id = 53) 
															 or (tur.dc_etapa_ensino like '%EJA%' and tur.dc_tipo_periodicidade = 'SEMESTRAL INICIO NO 1� SEMESTRE' and cal.cal_id = 54)
															 or (tur.dc_etapa_ensino like '%EJA%' and tur.dc_tipo_periodicidade = 'SEMESTRAL INICIO NO 2� SEMESTRE' and cal.cal_id = 55)
														     or (tur.dc_etapa_ensino not like '%infantil%' 
																 and tur.dc_etapa_ensino not like '%EJA%' and cal.cal_id = 52))
                   INNER JOIN GE_ESC_Escola es
	...
	select tur.esc_id
	  from GE_TUR_Turma tur
		   inner join GE_TUR_TurmaCurriculo tcr on tcr.tur_id = tur.tur_id and tcr.tcr_situacao <> 3 and tcr.cur_id in (249,250)
		   inner join GE_ACA_CalendarioAnual cal on cal.cal_id = tur.cal_id 
		   inner join _PARAMETROS par ON par.CHAVE = 'ANO_BASE' and cal.cal_ano = par.VALOR
...
BEGIN
    ...
    SELECT tur.tur_id, cur.cur_id, crr.crr_id, crp.crp_id, 1 AS tcr_prioridade, 1 AS tcr_situacao
     ...
     WHERE tmp.an_letivo = (SELECT CAST(VALOR as int) FROM _PARAMETROS WHERE CHAVE = 'ANO_BASE')
       ...
       and  ((crp.cur_id = 246 and crp.crp_ordem >= 6 and tmp.cd_escola not in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id = 246 and crp.crp_ordem < 6)
			  or (crp.cur_id = 239 and tmp.cd_escola not in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or crp.cur_id not in (239,241,246,249,250)
			)
    ...
    SELECT tur.tur_id, cur.cur_id, crr.crr_id, crp.crp_id, 1 AS tcr_prioridade, 1 AS tcr_situacao
     ...
     WHERE tmp.an_letivo = (SELECT CAST(VALOR as int) FROM _PARAMETROS WHERE CHAVE = 'ANO_BASE')
       and tmp.cd_tipo_turma <> 3
       and DEPARA_SERIE.cur_id in (SELECT cur_id FROM DEPARA_CURSOS_DIVIDIDOS)
	   and  ((crp.cur_id = 246 and crp.crp_ordem >= 6 and tmp.cd_escola in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id = 241 and tmp.cd_escola in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id in (241,249,250))
			)
    ...
    SELECT tur.tur_id, cur.cur_id, crr.crr_id, crp.crp_id, 1 AS tcr_prioridade, 1 AS tcr_situacao
     ...
     WHERE tmp.an_letivo = (SELECT CAST(VALOR as int) FROM _PARAMETROS WHERE CHAVE = 'ANO_BASE')
       ...
       AND  ((crp.cur_id = 246 and crp.crp_ordem >= 6 and tmp.cd_escola not in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id = 246 and crp.crp_ordem < 6)
			  or (crp.cur_id = 239 and tmp.cd_escola not in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id NOT IN (239,241,246))
			 )
    ...
    SELECT tur.tur_id, cur.cur_id, crr.crr_id, crp.crp_id, 1 AS tcr_prioridade, 1 AS tcr_situacao
     ...
     WHERE tmp.an_letivo = (SELECT CAST(VALOR as int) FROM _PARAMETROS WHERE CHAVE = 'ANO_BASE')
       AND tmp_turma_grade_mstech.dt_fim IS NULL
       and  ((crp.cur_id = 246 and crp.crp_ordem >= 6 and tmp.cd_escola in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id = 239 and tmp.cd_escola in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id in (241,249,250))
			)
	...
	--adicionado em 04/09/2017 pra tratar casos em que a turma de Recupera��o Paralela muda de nome no decorrer do ano 
	--(S�o exce��es mas decidimos tratar aqui pois n�o causam impacto no SGP)
	 ...
	 --por�m para o Ensino Infantil isso faz sentido, principalmente porque novas escolas e turmas aparecem a qualquer momento e as grades s�o menos complexas
		...
		select distinct cur_id, crr_id, tur.esc_id, uni_id, 1 as ces_id, '2026-01-01' as ces_vigenciaInicio,
		  ...
		  from GE_TUR_Turma tur
			   ...
			   on tur.tur_id = tcr.tur_id and tcr.cur_id in (235,236,237,238)
			   inner join GE_ESC_Escola esc
			   on tur.esc_id = esc.esc_id
...
BEGIN	
	...
	--------------------------------------------------------------------------------------------------------------------------------------
	--FOTO de territ�rios do saber e experiencias pedag�gicas antes de come�armos a alterar tuds
    ...
    SELECT DEPARA_TURMA.tur_id, dis.dis_id, 1 AS tud_verificador, tmpt.tur_codigo, dis.dis_nome AS tud_nome,
           ...
           tud_naoLancarFrequencia = case when dis.tds_id in (11,12) and crd.cur_id = 246 then 1 
										  when dis.tds_id = 10 and crd.crp_id < 6 then 1 
										  when dis.tds_id in (13,30,32) then 1 
										  else 0 end,
           tud_naoExibirFrequencia = case when dis.tds_id in (11,12) and crd.cur_id = 246 then 1 
										  when dis.tds_id = 10 and crd.crp_id < 6 then 1 
										  when dis.tds_id in (13,30,32) then 1 
										  else 0 end,
     ...
     WHERE dct.cd_tipo_turma not in (2, 3)
       ...
       AND crd_situacao = 1
       AND dis_situacao = 1
       AND (crd.cur_id not in (239,246)
           or (crd.cur_id in (239,246) and dis.dis_cargaHorariaTeorica <> 0))
    ...
    SELECT DEPARA_TURMA.tur_id, dis.dis_id, 1 AS tud_verificador, tmpt.tur_codigo, dis.dis_nome AS tud_nome,
      ...
      FROM #RelacaoTurma tmpt inner join tmp_DiarioClasse_turma dct
           ...
           (select tur_id, cur_id, crr_id, crp_id,
                   ROW_NUMBER() OVER (PARTITION BY tur_id ORDER BY CASE WHEN cur_id in (39,40,41,42,249,250,251) 
...
------------------------------------------------------------------
    --PEDRO: Corre��o para casos onde a tur_turmacurriculo mudou de curso... 
...
------------------------------------------------------------------------
	...
	--estava causando com que alunos com deficiencia n�o aparececem para docentes na posi��o 1 (o esperado era estarem na posicao 5) e ninguem mais usa a posi��o 5
	...
	--O Acerto do tud_id da Experiencia vai ser feito no final da TUR_TurmaDocente por causa da atribui��o de docentes mesmo
...
BEGIN
	/*
		Para execu��o desse procedimento � necess�rio que a Trigger TRG_TUR_TurmaDocente_Identity
		pois com a utiliza��o do comando MERGE, n�o h� tempo suficiente para o MERGE executar
		gerando um erro de viola��o de chave prim�ria.
	*/
...
--                        where cur_nome not like '%EJA%' and cur_situacao <> 3)
	...
	--- Ativa as atribui��es de docentes que j� existirem para as turmas de EJA----
	update GestaoPedagogica..TUR_TurmaDocente
           ...
           (select tud_id from GestaoPedagogica..TUR_TurmaRelTurmaDisciplina
             where tur_id in 
                   (select tcr.tur_id 
                     ...
                     --where modEnsino.tme_nome in ('CIEJA','EJA - Modular') --aguardando a publica��o do SGP pro EJA
					 where cur.cur_nome_abreviado in ('CIEJA', 'EJA MOD', 'EJA', 'EJA ESP') --aguardando a publica��o do SGP pro EJA
				    ))
    ...
    *************************************************/
	...
	--verifica as diferen�as comparando com a �ltima FOTO e joga na temporaria:
	...
	select tur.tur_id, tudTer.tud_id, u.ter_id as ter_idAntigo, ter.ter_id as ter_idNovo, 
	 ...
	 where tudTer.tud_tipo = 19 -- territorios do saber que � o OBRIGATORIO
	   ...
	   and (isnull(u.ter_id,-1) <> isnull(ter.ter_id,-1) 
		    or isnull(u.doc_id,-1) <> isnull(tdt.doc_id,-1))  -- filtrando apenas quando n�o encontrou ou tem alguma diferen�a quanto a ultima foto
	 order by tudTer.tud_id
	
	IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na inser��o de dados na tabela #territoriosDiferencas'
       ROLLBACK
       RETURN
    END
	...
	-- adiciona as diferen�as agrupando por experiencias e joga na nova temp
	...
	IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na inser��o de dados na tabela #DiferencasAgrupadas 1'
       ROLLBACK
       RETURN
    END
	...
	IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na inser��o de dados na tabela #DiferencasAgrupadas 2'
       ROLLBACK
       RETURN
    END
	...
	IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na inser��o de dados na tabela #DiferencasAgrupadas 3'
       ROLLBACK
       RETURN
    END
	...
	IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na inser��o da #tmp_TUR_TurmaDisciplina_novaExp 1'
       ROLLBACK
       RETURN
    END
	...
	IF @@ERROR <> 0
    BEGIN
       PRINT 'Erro na inser��o da #tmp_TUR_TurmaDisciplina_novaExp 2'
       ROLLBACK
       RETURN
    END
    ...
    *************************************************/
	...
	--deixei para colocar no final, para n�o atrapalhar a cria��o do registro na TTE no c�digo acima
...
BEGIN
    ...
    -- Armazena nas vari�veis os tipos de documenta��o pra CPF e RG
	...
	SELECT @gru_idResponsavel = gru_idUsadoIntegracao FROM DEPARA_GRUPOS_INTEGRACAO 
     WHERE nomeUsadoIntegracao = 'Respons�vel' and sis_id = 196
    ...
    -- pois esses n�o foram inclu�dos pela importa��o.
    INSERT INTO @UsuarioTable (usu_id, usu_login)
    SELECT usu.usu_id, usu.usu_login
      FROM SSO_SYS_Usuario usu
           INNER JOIN SSO_SYS_UsuarioGrupo usg ON usu.usu_id = usg.usu_id
    ...
    INSERT INTO PackageTaskLog (PackageLogID, SourceName, SourceID, StartDateTime)
		...
		--A��es poss�veis:
			...
			-- PESSOAS_DIFERENTES_NAO_IMPORTAR_RESPONSAVEL: vou limpar os campos de respons�vel e n�o importar at� que seja acertado e n�o caia mais nessa categoria. Importante, essa categoria s� � cadastrada pela importa��o
		...
		--teoricamente n�o � pra ter nenhum caso, mas deixo o c�digo aqui para casos que forem acertados na m�o, ou altera��o do nome do servidor, ou algo assim
               ...
               AND pes_idFiliacaoMae NOT IN
                   (SELECT pes_id FROM SSO_PES_PessoaDocumento PSD
                     WHERE PSD.pes_id = IMP_alunos_pais.pes_idFiliacaoMae
                       and PSD.tdo_id = @tdo_id_cpf
					   AND PSD.psd_situacao = 1)) DOC
	    WHERE  cast(codigo_cpf_responsavel as varchar(20))  NOT IN (select psd_numero from SSO_PES_PessoaDocumento)
    ...
    -- Cadastra as permiss�es dos alunos para o Boletim On Line
    IF @gru_idAluno IS NOT NULL
    BEGIN
    ...
    -- Cadastra as permiss�es dos alunos para o Serap
       ...
       SELECT usu.usu_id, @gru_idAlunoSerap AS gru_id, 1
         FROM IMP_alunos_pais
              INNER JOIN GE_ACA_AlunoCurriculo alc with (nolock)
               on IMP_alunos_pais.alu_codigo = alc.alc_matricula
              and 3 <> alc.alc_situacao
			  and cur_id not in (235,236,237,238) -- ensino infantil
              INNER JOIN GE_ACA_Aluno alu with (nolock)
              on alc.alu_id = alu.alu_id
              inner join SSO_PES_Pessoa pes with (nolock)
              on alu.pes_id = pes.pes_id
    ...
    -- Cadastra as permiss�es dos respons�veis para o Boletim On Line
    ...
    IF OBJECT_ID('tempdb..#alunos_mtr') > 0 
       DROP TABLE #alunos_mtr
    
    CREATE TABLE #alunos_mtr
      (pes_id UNIQUEIDENTIFIER,
	   ent_id UNIQUEIDENTIFIER,
	   alu_codigo INT,
	   alu_codigoIntegracao INT,
	   alu_ativo INT,
	   alu_dataCriacao DATETIME,
	   alu_dataAlteracao DATETIME,
       linha INT)
	   
	INSERT INTO #alunos_mtr
	SELECT tmp.pes_id,
		@ent_id AS ent_id,
		tmp.alu_codigo,
		1 AS alu_codigoIntegracao,
		1 AS alu_ativo, 
		GETDATE(), 
		GETDATE(),
		ROW_NUMBER() OVER(PARTITION BY tmp.pes_id ORDER BY mtr.cl_dt_inclusao DESC) as linha
	FROM IMP_alunos_pais tmp
		INNER JOIN tmp_DiarioClasse_aluno mtr on tmp.alu_codigo = mtr.cl_alu_codigo
	...
	SELECT mtr.pes_id,
		mtr.ent_id,
		mtr.alu_codigo,
		mtr.alu_codigoIntegracao,
		mtr.alu_ativo,
		mtr.alu_dataCriacao,
		mtr.alu_dataAlteracao
	FROM #alunos_mtr mtr
	WHERE mtr.linha = 1	   
    ...
    USING (SELECT t.alu_id,
				t.alr_id,
				t.tra_id,
				t.pes_id,
				t.alr_principal,
				t.alr_constaCertidaoNascimento,
				t.alr_moraComAluno,
				t.alr_situacao,
				t.alr_dataCriacao,
				t.alr_dataAlteracao,
				t.alr_omitidoFormaLei
			FROM (
				SELECT IMP_alunos_pais.alu_id,
					ISNULL(alr.alr_id, ISNULL(alu_rsp.alr_id,0) + 1) AS alr_id,
					2 AS tra_id,
					pes_idFiliacaoPai AS pes_id,
					alr_principal = case IMP_alunos_pais.tipo_responsavel when 2 THEN 1 ELSE 0 END,
					1 AS alr_constaCertidaoNascimento,
					1 AS alr_moraComAluno,
					1 AS alr_situacao,
					GETDATE() AS alr_dataCriacao,
					GETDATE() AS alr_dataAlteracao,
					1 AS alr_omitidoFormaLei,
					ROW_NUMBER() OVER(PARTITION BY IMP_alunos_pais.pes_id ORDER BY tmp_DiarioClasse_aluno.cl_dt_inclusao DESC) as linha
				FROM IMP_alunos_pais 
					INNER JOIN tmp_DiarioClasse_aluno ON IMP_alunos_pais.alu_codigo = tmp_DiarioClasse_aluno.cl_alu_codigo
					LEFT JOIN GE_ACA_AlunoResponsavel alr ON IMP_alunos_pais.alu_id = alr.alu_id
					...
					LEFT JOIN (SELECT alu_id, MAX(alr_id) alr_id
								...
								group by alu_id) alu_rsp ON IMP_alunos_pais.alu_id = alu_rsp.alu_id
				WHERE pes_idFiliacaoPai IS NOT NULL
				) t
				where t.linha = 1
			) AS _source
    ...
    USING (SELECT t.alu_id,
			...
			FROM (
				...
				and pes_idFiliacaoRsp IS NOT NULL
				) t
				where t.linha = 1
			) AS _source
     ON _target.alu_id = _source.alu_id
			...
			FROM (
				...
				FROM IMP_alunos_pais 
					...
					LEFT JOIN (SELECT alu_id, MAX(alr_id) alr_id
								...
								where alr_situacao <> 3
								group by alu_id) alu_rsp ON IMP_alunos_pais.alu_id = alu_rsp.alu_id
				WHERE IMP_alunos_pais.tipo_responsavel = 3) t
				where t.linha = 1
			) AS _source
     ON _target.alu_id = _source.alu_id
    AND _target.alr_id = _source.alr_id
...
select cl_codigo, cl_alu_codigo, cl_tur_codigo, cl_datst, cl_cham, des_status, cl_nota,
  ...
  from BD_PRODAM..v_alunos_turma_matriculados_historico_movimentacao hm inner join
	   ...
	   INNER JOIN tmpTipoSerieEOL AS serie -- ADD em 13/09 Alunos "transferidos" para o EJA, continuavam ativos no SGP 
			   ON serie.cd_serie_eol98 = tur.cd_serie_eol98 -- E-mail: Atualiza��o SGP - EMEF Josefa Nic�cio - fgonsales@PREFEITURA.SP.GOV.BR>
 where tur.cd_tipo_turma = 1
   AND tur.cd_etapa_ensino IN (1,2,3,4,5,6,9,10,11,13,14) --filtrar apenas os cursos tratados pela integra��o
...
--BLOCO QUE FAZ AJUSTE ESPEC�FICO PARA O ENSINO M�DIO E ENSINO M�DIO T�CNICO
--A REGRA DECIDIDA EM CONVERSA COM CARLA/HAILA/JULIANO FOI: 
--SE O ALUNO EST� COM A ULTIMA MATRICULA NO T�CNICO: VERIFICAR SE ELE TEM ALGUMA MATR�CULA NO M�DIO REGULAR E DEIXAR ELE NO M�DIO REGULAR 
--SE O ALUNO EST� COM A ULTIMA MATRICULA NO T�CNICO E N�O TEM MATR�CULA NO M�DIO REGULAR, MATRICULAR ELE NO T�CNICO
               ...
               (select cl_codigo, cl_alu_codigo, cl_tur_codigo, cl_datst, cl_cham, cl_st, des_status,
                 ...
                 from (select cl_codigo, cl_alu_codigo, LEFT(cl_cham,3) AS cl_cham, cl_tur_codigo, 
                              ...
                              ROW_NUMBER() OVER (PARTITION BY cl_alu_codigo
                                           ...
                                           -- o julio tinha add essa linha acima em 29/02, mas estava causando in�meros chamados em que pegava a situa��o errada do aluno (mais antiga) por causa do cl_codigo
...
SELECT cd_escola, cd_turma_escola, tne_nome, tme_nome, cur_codigo,
  ...
  FROM (SELECT tal.cd_escola, tal.cd_turma_escola, CONVERT(VARCHAR(100),
          ...
          FROM BD_PRODAM..v_turma_MSTECH tal
			   INNER JOIN GE_ESC_Escola esc on esc.esc_codigo = tal.cd_escola 
										   AND esc.esc_situacao <> 3
										   --and esc.esc_controleSistema = 1 --filtro para funcionar com menos escolas em homologa��o
...
BEGIN     
    ...
    -- Deleta a rela��o UsuarioGrupoUA para os registros que est�o na tmp de servidores importados automaticamente
    DELETE ugu
      FROM SSO_SYS_UsuarioGrupoUA ugu
           INNER JOIN SSIS_LoginImportado lo
            ON ugu.usu_id = lo.usu_id
...
BEGIN
    ...
    -- para todos os registros na SYS_UnidadeAdministrativa que tiverem rela��o com um registro ativo no EOL
...
BEGIN
    ...
    SET @tua_id_dre = (SELECT tua_id FROM CoreSSO..SYS_TipoUnidadeAdministrativa WHERE LOWER(tua_nome) = 'diretoria regional de educa��o')
    ...
    --Cria tabela de diretorias para valida��o apenas de distritos
...
BEGIN
	...
	END 
	--update adicionado conforme solicita��o em 14/05/2015, para resolvermos problemas de diferen�as entre a base do portal e do core
	--MERGE adicionado conforme solicita��o em 18/06/2015
...
BEGIN
	...
	(		
		...
		WHERE   cd_tipo_turma = 1 --REGULAR 
				...
				AND cd_serie_eol98 IS NOT NULL
		GROUP BY 
...
AS
  BEGIN
	...
	-- Insere ou atualiza infoma��es na tabela TurmaAlunos
...
-- ID do sistema
        ...
        WHERE   sis_nome = 'Sistema de Avalia��o'
...
BEGIN
	...
	VALUES ('Diretoria Regional de Educa��o', 1, GETDATE(), GETDATE(), 0)
END

...
Declare
	...
	--7 4º ao 5º ano;				Interdisciplinar
	--10 6º ano;					Interdisciplinar
...
-- na documentação dos dados não possui mapeamento de algumas tabelas de fechamento, 
--e como fechamento é baseado em 4 Bimestres foi feito isso

AND alu_ava_tur_dis.atd_avaliacao IS NOT NULL
AND alu_ava_tur_dis.atd_avaliacao NOT IN
('NS','P','S','F (Frequente)','NF (Não Frequente)','Aprovado',',','.','-2146826281','-2146826273','7,0,')

AND dbo.Utility_fn_FormataNota(atd_avaliacao) <= CONVERT(Decimal(15,1), 5)
AND fav.fav_id In(@fav_id,@Fav_id_2)
AND cal.cal_ano = @ano

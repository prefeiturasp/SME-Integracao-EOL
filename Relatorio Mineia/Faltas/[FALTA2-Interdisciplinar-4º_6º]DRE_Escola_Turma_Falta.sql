...
DECLARE
	...
	,@dataIntervaloFim	  DATE = '2019-04-30'
	 ...
	 --@dataIntervaloInicio	DATE = DateAdd(Day,- 15,GetDate())
  --  ,@dataIntervaloFim		DATE = Cast(GetDate() As date)

	,@fav_id INT = 10
	,@fav_id_2 INT = 7 

	--resultado:199042 16min47seg
...
AND tau_dada.tau_data BETWEEN @dataIntervaloInicio 
...
FROM MTR_MatriculaTurma mtu
	...
	WHERE tur.fav_id In(@fav_id,@fav_id_2)
	 
	AND tau.tau_data BETWEEN @dataIntervaloInicio 
	AND @dataIntervaloFim 	
	AND tau.tau_situacao <> 3
	AND Left(RTrim(LTrim(tud.tud_codigo)),1) IN('4','5','6')

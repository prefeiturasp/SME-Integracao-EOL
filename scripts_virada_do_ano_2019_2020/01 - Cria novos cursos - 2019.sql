...
select cur_id + 20 as cur_id , ent_id, cur_padrao, tne_id, tme_id, cur_codigo, 
	   replace(cur_nome,'2019','2020'), cur_nome_abreviado, cur_exclusivoDeficiente, 
	   '2020-01-01' as cur_vigenciaInicio, cur_efetivacaoSemestral, cur_situacao, GETDATE(), GETDATE()
  from ACA_Curso
 where cur_nome like '2019%'
...
select cur_id + 20 as cur_id, crr_id, crr_codigo, crr_regimeMatricula, crr_periodosNormal, crr_diasLetivos,
  ...
  from ACA_Curriculo
 where cur_id in (select cur_id from ACA_Curso where cur_nome like '2019%')
...
select dis.dis_id, dis_nome, dis_nomeAbreviado, tds_id, dis_codigo, crd_tipo, cur_id + 20, crp_id, dis_cargaHorariaTeorica
  from ACA_Disciplina dis
	   inner join ACA_CurriculoDisciplina crd on crd.dis_id = dis.dis_id 
											 and crd.cur_id in (select cur_id from ACA_Curso where cur_nome like '2019%')
...
update Manutencao..DEPARA_CURSOS_DIVIDIDOS set cur_id = cur_id + 20 where cur_id in (select cur_id from ACA_Curso where cur_nome like '2019%')

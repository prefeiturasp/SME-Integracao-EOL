USE [Manutencao]
GO
/****** Object:  StoredProcedure [dbo].[STP_GestaoAvaliacao_Import]    Script Date: 31/01/2023 20:40:00 ******/
SET ANSI_NULLS ON
GO
...
AS
BEGIN
    declare '6CF424DC-8EC3-E011-9B36-00155D033206' uniqueidentifier, 2023 int, @SourceID UNIQUEIDENTIFIER, @PackageLogID INT
    
    select 2023 = VALOR from _PARAMETROS where CHAVE = 'ANO_BASE'
	--set 2023 = 2015
    select '6CF424DC-8EC3-E011-9B36-00155D033206'  from CoreSSO..SYS_Entidade where ent_sigla = 'SMESP' and ent_situacao = 1
    
    if '6CF424DC-8EC3-E011-9B36-00155D033206' is null
    begin
    ...
    USING (select cal_id, ent_id, cal_padrao, cal_ano, cal_descricao, cal_dataInicio, cal_dataFim,
            ...
            where cal_ano = 2023
              and ent_id = '6CF424DC-8EC3-E011-9B36-00155D033206'
    ...
    USING (select alu.alu_id, alu.pes_id, pes.pes_nome, alu.ent_id, alc.alc_matricula,
            ...
            where alu.ent_id = '6CF424DC-8EC3-E011-9B36-00155D033206'
    ...
    WHEN NOT MATCHED BY SOURCE AND Destino.cur_situacao <> 3 AND --tme_id NOT IN (4,6,7,8)
	THEN
	...
	WHERE mtu.mtu_situacao = 3 and mtu.mtu_dataAlteracao = GETDATE()
    ...
    USING (select doc.doc_id, pes.pes_nome, doc.doc_situacao,
            ...
            where doc.doc_situacao <> 3
              ...
              and col.ent_id = '6CF424DC-8EC3-E011-9B36-00155D033206'
              and pes.pes_situacao <> 3
   ...
   -- BEGIN
        ...
        WHEN NOT MATCHED THEN
             ...
             VALUES (_source.usu_id, _source.gru_id, '6CF424DC-8EC3-E011-9B36-00155D033206', _source.uad_id);

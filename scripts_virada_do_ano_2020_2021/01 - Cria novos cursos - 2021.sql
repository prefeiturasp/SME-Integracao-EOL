...
select cur_id + 20 as cur_id , ent_id, cur_padrao, tne_id, tme_id, cur_codigo, 
	   replace(cur_nome,'2021','2021'), cur_nome_abreviado, cur_exclusivoDeficiente, 
	   '2020-01-01' as cur_vigenciaInicio, cur_efetivacaoSemestral, cur_situacao, GETDATE(), GETDATE()
  from ACA_Curso
 where cur_nome like '2020%'
...
select cur_id + 20 as cur_id, crr_id, crr_codigo, crr_regimeMatricula, crr_periodosNormal, crr_diasLetivos,
  ...
  from ACA_Curriculo
 where cur_id in (select cur_id from ACA_Curso where cur_nome like '2020%')

...
/* TRATAMENTO EXCLUSIVO PARA OS MTDS DAS EXPERIÊNCIAS PEDAGÓGICAS DE TERRITÓRIOS DO SABER */
    ...
    update mtd
      ...
      from GE_MTR_MatriculaTurmaDisciplina mtd
           ...
           inner join GE_TUR_Turma tur with (nolock)
           on trtd.tur_id = tur.tur_id
		   inner join GE_ACA_CalendarioAnual cal with (nolock) --12/04/2018 - Não excluir matriculas inativas de anos anteriores
           ON tur.cal_id = cal.cal_id
	   ...
	   from (select mtu.alu_id, mtu.mtu_id 
			   from GestaoPedagogica..ACA_AlunoCurriculo alc
				    ...
				    inner join GestaoPedagogica..MTR_MatriculasBoletim mb 
						   ...
						   and mb.mtu_id is not null
					INNER JOIN GestaoPedagogica..MTR_MatriculaTurma mtu2 on mtu2.alu_id = alc.alu_id and mtu2.mtu_id = mb.mtu_id
																			  and mtu2.cur_id not in (231,232) --cursos sem TUD E MTD
			  where alc.alc_situacao <> 3
				and mtu.mtu_situacao <> 3 --and alc.esc_id between 500 and 600
			    and not exists (select mb.alu_id from GestaoPedagogica..MTR_MatriculasBoletimDisciplina mb 
								 where mb.alu_id = mtu.alu_id
...
BEGIN
	...
	WITH CteDestinationTurma AS (
		...
		WHERE tur_id IN ( 
			...
			--where c.tme_id NOT IN (4,6,7,8) Retirando exclusão de turmas EJA para que entrem no serap
		)
	)
    ...
    USING (select tur.tur_id, tur.esc_id, tur.tur_codigo, tur.tur_descricao, tur.cal_id, trn.ttn_id,
            ...
            where tur.tur_situacao <> 3
			  ...
			  and tur_tipo IN (1,6)
			 -- and c.tme_id NOT IN (4,6,7,8)  Retirando exclusão de turmas EJA para que entrem no serap
	...
	(
		...
		--WHERE aluno.alu_id NOT IN ( SELECT alu_id FROM #CteIdDosAlunosDoEjaeCieja )  Retirando exclusão de turmas EJA para que entrem no serap
	)
    ...
    USING (select alu.alu_id, alu.pes_id, pes.pes_nome, alu.ent_id, alc.alc_matricula,
			...
			--  and alu.alu_id NOT IN ( SELECT alu_id FROM #CteIdDosAlunosDoEjaeCieja ) Retirando exclusão de turmas EJA para que entrem no serap
	...
	update GestaoPedagogica..ACA_CurriculoEscola set ces_situacao = 3, ces_dataAlteracao = GETDATE() where cur_id in (231,232) and ces_situacao <> 3
    ...
    BEGIN
                          ...
                          from (select prf.rf, gc.cd_escola
                                  from BD_PRODAM..v_cadastro_professor prf
									   ...
									   inner join DEPARA_SERIE dep on dep.cd_serie_ensino = tm.cd_serie_ensino
											  and ((dep.cur_id in (223,225,226) and dep.crp_ordem > 2) --EF (menos o de 4 horas) a partir do terceiro ano (a pedido do Hygor por email em 05/05/2016)
												    or (dep.cur_id in (39,40,41,42,227,228,229) and dep.crp_ordem in (2,4,6,8))) --EJA apenas algumas séries (a pedido do Hygor por email em 05/05/2016)
                                       inner join BD_PRODAM..v_unidade_educacao_dados_gerais esc
                                       on gc.cd_escola = esc.cd_unidade_educacao
                                 where esc. dc_tipo_unidade_educacao = 'ESCOLA' 
                                   and esc.sg_tp_escola in ('EMEF','EMEFM','CEU EMEF','EMEBS','CIEJA',
	   ...
	   -- 16/03/2024  **
		...
		WHERE  tua_nome = 'Diretoria Regional de Educação';
...
FROM    ( SELECT    cd_serie_ensino ,
        ...
        INNER JOIN GE_ACA_Curso cur WITH ( NOLOCK ) 
				ON cur.cur_nome COLLATE DATABASE_DEFAULT = _out.crr_nome COLLATE DATABASE_DEFAULT
				   or (cur.cur_id = 217 and _out.crr_nome = 'Ensino Fundamental Especial de 9 Anos')
				   or (cur.cur_id = 219 and _out.crr_nome = 'Ensino Médio Regular')
...
GROUP BY 
	...
	UPDATE #curCurriculoPeriodo SET cur_id = 229 where cur_id = 41
	...
	SELECT crp.crp_ordem, CAST(crp.crp_ordem as varchar(1))+'° ano', 5, crr_nome, 2, 5, 
		   CASE when crp.crp_ordem IN (1,2,3) THEN 'Ciclo de Alfabetização'  
		        ...
		        case when crr.cur_id = 218 then 249 + crp_ordem
				else 
						case crp_ordem 
	 ...
	 where div.cur_id not in (217,219,220,227,228,229,41,42,231,232)
	...
	delete from #curCurriculoPeriodo where cur_id in (213,214,215,216)
	
	INSERT INTO #curCurriculoPeriodo
	values (1, 'Fundamental I Libras', 13, 'Ensino Fundamental 9 Anos',                  2, 5, 'CICLO I',  0, 1, 220, 1, 1, 1, 0, 226)
	    ,(1, 'Fundamental I Libras', 13, 'Ensino Fundamental 9 Anos',                    2, 5, 'CICLO I',  0, 1, 221, 1, 1, 1, 0, 226)
		...
		,(1, 'EJA ALFABETIZACAO I' ,  3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 227, 1, 8, 1, 0, 274)
		,(2, 'EJA ALFABETIZACAO II',  3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 227, 1, 8, 2, 0, 275)	
		,(3, 'EJA BASICA I',		  3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 227, 1, 8, 3, 0, 278)	
		,(4, 'EJA BASICA II',		  3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 227, 1, 8, 4, 0, 280)	
		,(5, 'EJA COMPLEMENTAR I',    3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 227, 1, 8, 5, 0, 246)	
		,(6, 'EJA COMPLEMENTAR II',   3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 227, 1, 8, 6, 0, 247)	
		,(7, 'EJA FINAL I',			  3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 227, 1, 8, 7, 0, 248)	
		,(8, 'EJA FINAL II',		  3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 227, 1, 8, 8, 1, 249)	
		                                                                                                          
		,(1, 'EJA ALFABETIZACAO I',   3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 227, 1, 8, 1, 0, 276)
		,(2, 'EJA ALFABETIZACAO II',  3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 227, 1, 8, 2, 0, 277)	
		,(3, 'EJA BASICA I',		  3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 227, 1, 8, 3, 0, 279)	
		,(4, 'EJA BASICA II',		  3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO I',  0, 1, 227, 1, 8, 4, 0, 281)	
		,(5, 'EJA COMPLEMENTAR I',    3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 227, 1, 8, 5, 0, 265)	
		,(6, 'EJA COMPLEMENTAR II',   3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 227, 1, 8, 6, 0, 266)	
		,(7, 'EJA FINAL I',			  3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 227, 1, 8, 7, 0, 267)	
		,(8, 'EJA FINAL II',		  3, 'EJA - Educação de Jovens e Adultos',           2, 5, 'CICLO II', 0, 1, 227, 1, 8, 8, 1, 268)	
		                                                                                
		,(3, 'EJA BASICA I',		  11, 'EJA - Educação de Jovens e Adultos',          2, 5, 'CICLO I',  0, 1, 228, 1, 8, 3, 0, 292)	
		,(4, 'EJA BASICA II',		  11, 'EJA - Educação de Jovens e Adultos',          2, 5, 'CICLO I',  0, 1, 228, 1, 8, 4, 0, 294)	
		,(4, 'EJA BASICA II',		  11, 'EJA - Educação de Jovens e Adultos',          2, 5, 'CICLO I',  0, 1, 228, 1, 8, 4, 0, 295)	
		,(5, 'EJA COMPLEMENTAR I',    11, 'EJA - Educação de Jovens e Adultos',          2, 5, 'CICLO II', 0, 1, 228, 1, 8, 5, 0, 261)	
		,(6, 'EJA COMPLEMENTAR II',   11, 'EJA - Educação de Jovens e Adultos',          2, 5, 'CICLO II', 0, 1, 228, 1, 8, 6, 0, 270)	
		,(6, 'EJA COMPLEMENTAR II',   11, 'EJA - Educação de Jovens e Adultos',          2, 5, 'CICLO II', 0, 1, 228, 1, 8, 6, 0, 262)	
		,(7, 'EJA FINAL I',			  11, 'EJA - Educação de Jovens e Adultos',          2, 5, 'CICLO II', 0, 1, 228, 1, 8, 7, 0, 263)	
		,(8, 'EJA FINAL II',		  11, 'EJA - Educação de Jovens e Adultos',          2, 5, 'CICLO II', 0, 1, 228, 1, 8, 8, 1, 272)	
		
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 213, 1, 6, 5, 0, 23)	
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 213, 1, 6, 5, 0, 24)	
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 213, 1, 6, 5, 0, 225)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 213, 1, 6, 6, 1, 118)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 213, 1, 6, 6, 1, 25)	
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 213, 1, 6, 6, 1, 26)	
		                                                                                 
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 214, 1, 6, 5, 0, 23)	
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 214, 1, 6, 5, 0, 24)	
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 214, 1, 6, 5, 0, 225)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 214, 1, 6, 6, 1, 118)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 214, 1, 6, 6, 1, 25)	
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 214, 1, 6, 6, 1, 26)	
		                                                                                 
		,(4, 'Mini grupo II',          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 215, 1, 6, 4, 0, 28)	
		,(4, 'Mini grupo II',          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 215, 1, 6, 4, 0, 29)
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 215, 1, 6, 5, 0, 23)	
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 215, 1, 6, 5, 0, 24)
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 215, 1, 6, 5, 0, 225)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 215, 1, 6, 6, 1, 118)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 215, 1, 6, 6, 1, 25)	
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 215, 1, 6, 6, 1, 26)	
		                                                                                 
		,(1, 'Berçario I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 1, 0, 1)
		,(1, 'Berçario I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 1, 0, 3)
		,(2, 'Berçario II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 2, 0, 4)
		,(2, 'Berçario II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 2, 0, 6)
		,(3, 'Mini grupo I' ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 3, 0, 11)
		,(3, 'Mini grupo I' ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 3, 0, 27)
		,(4, 'Mini grupo II',          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 4, 0, 28)
		,(4, 'Mini grupo II',          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 4, 0, 29)
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 5, 0, 23)	
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 5, 0, 24)
		,(5, 'Infantil I'   ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 5, 0, 225)
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 6, 1, 118)		
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 6, 1, 25)	
		,(6, 'Infantil II'  ,          1, 'Ensino infantil',                             2, 5, 'Infantil', 0, 1, 216, 1, 6, 6, 1, 26)
		
		,(1, '1º Modulo'   ,          14, 'Técnico Médio',                               2, 5, 'SEM CICLO', 0, 1, 231, 1, 3, 1, 0, 194)
		,(2, '2º Modulo'   ,          14, 'Técnico Médio',                               2, 5, 'SEM CICLO', 0, 1, 231, 1, 3, 2, 0, 196)
		,(3, '3º Modulo'   ,          14, 'Técnico Médio',                               2, 5, 'SEM CICLO', 0, 1, 231, 1, 3, 3, 1, 198)
                                                                                        
		,(1, '1º Modulo'   ,          14, 'Técnico Médio',                               2, 5, 'SEM CICLO', 0, 1, 231, 1, 3, 1, 0, 282)
		,(2, '2º Modulo'   ,          14, 'Técnico Médio',                               2, 5, 'SEM CICLO', 0, 1, 231, 1, 3, 2, 0, 284)
		,(3, '3º Modulo'   ,          14, 'Técnico Médio',                               2, 5, 'SEM CICLO', 0, 1, 231, 1, 3, 3, 1, 286)
		                                                                                
		,(1, '1º Modulo'   ,          14, 'Técnico Médio',                               2, 5, 'SEM CICLO', 0, 1, 231, 1, 3, 1, 0, 283)
		,(2, '2º Modulo'   ,          14, 'Técnico Médio',                               2, 5, 'SEM CICLO', 0, 1, 231, 1, 3, 2, 0, 285)
		,(3, '3º Modulo'   ,          14, 'Técnico Médio',                               2, 5, 'SEM CICLO', 0, 1, 231, 1, 3, 3, 1, 287)

		,(1, '1º Ano'   ,              9, 'Ensino Médio Magistério',                     2, 5, 'SEM CICLO', 0, 1, 232, 1, 4, 1, 0, 101)
		,(2, '2º Ano'   ,              9, 'Ensino Médio Magistério',                     2, 5, 'SEM CICLO', 0, 1, 232, 1, 4, 2, 0, 103)
		,(3, '3º Ano'   ,              9, 'Ensino Médio Magistério',                     2, 5, 'SEM CICLO', 0, 1, 232, 1, 4, 3, 0, 105)
		,(4, '4º Ano'	,              9, 'Ensino Médio Magistério',                     2, 5, 'SEM CICLO', 0, 1, 232, 1, 4, 4, 1, 107)	
	...
	FROM (SELECT distinct grade.*, gc.qt_aula_semanal, depSer.crp_id, depSer.cur_id, depDis.cd_componente_curricular, depDis.dis_nome
		    FROM tmp_grade_mstech grade 
			   ...
			   INNER JOIN DEPARA_SERIE depSer 
					   ON depSer.cd_serie_ensino = grade.cd_serie_ensino 
					  and depSer.cur_id in (224,217,219,220,227,228,229,41,42,216)
		 WHERE (dt_fim_validade is null or dt_fim_validade >= GETDATE())
	   ) a 
	...
	UPDATE DEPARA_CURSO_GRADES 
	   SET cur_id = 223, crr_id = 1, crp_id = dep.crp_id
	  FROM DEPARA_CURSO_GRADES
	...
	UPDATE DEPARA_CURSO_GRADES 
	   SET cur_id = CASE WHEN soma_horas > 40 and cd_etapa_ensino = 5 THEN 226
					 WHEN soma_horas between 32 and 40 and cd_etapa_ensino = 5 and dep.crp_ordem <= 5 THEN 225
					 WHEN soma_horas between 30 and 40 and cd_etapa_ensino = 5 and dep.crp_ordem >= 6 THEN 225
					 WHEN soma_horas < 32 and cd_etapa_ensino = 5 and dep.crp_ordem <= 5 THEN 224
					 WHEN soma_horas < 30 and cd_etapa_ensino = 5 and dep.crp_ordem >= 6 THEN 224
					 WHEN soma_horas >= 31 and cd_grade not in (3541,3542,4285) and cd_etapa_ensino = 13 and dep.crp_ordem <= 6 THEN 218
					 WHEN soma_horas >= 30 and cd_grade not in (3541,3542,4285) and cd_etapa_ensino = 13 and dep.crp_ordem >= 7 THEN 218
					 WHEN soma_horas <= 30 and cd_grade not in (3541,3542,4285) and cd_etapa_ensino = 13 and dep.crp_ordem <= 6 THEN 217
					 WHEN soma_horas <= 29 and cd_grade not in (3541,3542,4285) and cd_etapa_ensino = 13 and dep.crp_ordem >= 7 THEN 217
					 WHEN soma_horas >= 51 and cd_etapa_ensino = 1 THEN 194
					 WHEN soma_horas between 37 and 50 and cd_etapa_ensino = 1 THEN 215
					 WHEN soma_horas between 26 and 36 and cd_etapa_ensino = 1 THEN 214
					 WHEN soma_horas <= 25 and cd_etapa_ensino = 1 THEN 213
					 WHEN cd_etapa_ensino = 6 THEN 197
					 WHEN DEPARA_CURSO_GRADES.cd_serie_ensino = 226 and cd_grade not in (4285) THEN 220
					 WHEN DEPARA_CURSO_GRADES.cd_serie_ensino = 226 and cd_grade = 4285 THEN 221
				 END, 
	 ...
	 WHERE dep.cur_id in (select cur_id from DEPARA_CURSOS_DIVIDIDOS) 
	   ...
	   and dep.cur_id not in (227,228,229,41,42) -- já não entrariam por causa da DEPARA_CURSOS_DIVIDIDOS mas só pra garantir
	...
	UPDATE DEPARA_CURSO_GRADES 
	  ...
	  FROM DEPARA_CURSO_GRADES
		   INNER JOIN DEPARA_SERIE dep ON dep.cd_serie_ensino = DEPARA_CURSO_GRADES.cd_serie_ensino
	 WHERE dep.cur_id in (227,228,229,41,42,231,232)
	   and DEPARA_CURSO_GRADES.cur_id is null
	...
	UPDATE DEPARA_CURSO_GRADES 
	 ...
	 WHERE dep.cur_id in (224,225,226)   
     ...
     UPDATE DEPARA_CURSO_GRADES
		SET fl_edFisica = 0 
	  WHERE cur_id in (218,220,221,223,225,226,41,42,213,214,215,216,231,232)
	  
	 UPDATE DEPARA_CURSO_GRADES
		SET fl_edFisica = 0 
	  WHERE cur_id = 224 and crp_id < 6
...
BEGIN
      ...
      FROM (SELECT DEPARA_TURMA.tur_id, es.esc_id, uni.uni_id,
              ...
              FROM tmp_DiarioClasse_turma tur 
				   INNER JOIN GE_ACA_CalendarioAnual cal ON cal.cal_ano = tur.an_letivo and cal.cal_situacao = 1
														AND ((tur.dc_etapa_ensino like '%infantil%' and cal.cal_id = 48) 
															 or (tur.dc_etapa_ensino like '%EJA%' and tur.dc_tipo_periodicidade = 'SEMESTRAL INICIO NO 1º SEMESTRE' and cal.cal_id = 49)
															 or (tur.dc_etapa_ensino like '%EJA%' and tur.dc_tipo_periodicidade = 'SEMESTRAL INICIO NO 2º SEMESTRE' and cal.cal_id = 50)
														     or (tur.dc_etapa_ensino not like '%infantil%' 
																 and tur.dc_etapa_ensino not like '%EJA%' and cal.cal_id = 47))
                   INNER JOIN GE_ESC_Escola es
	...
	select tur.esc_id
	  from GE_TUR_Turma tur
		   inner join GE_TUR_TurmaCurriculo tcr on tcr.tur_id = tur.tur_id and tcr.tcr_situacao <> 3 and tcr.cur_id in (227,228)
		   inner join GE_ACA_CalendarioAnual cal on cal.cal_id = tur.cal_id 
		   inner join _PARAMETROS par ON par.CHAVE = 'ANO_BASE' and cal.cal_ano = par.VALOR
...
BEGIN
	...
	insert into tmpTipoSerieEOL (cd_serie_eol98)
	 select distinct cd_serie_eol98 from BD_PRODAM..v_turma_MSTECH tm
	  where an_letivo = 2025 and cd_serie_ensino is not null
    ...
    SELECT tur.tur_id, cur.cur_id, crr.crr_id, crp.crp_id, 1 AS tcr_prioridade, 1 AS tcr_situacao
     ...
     WHERE tmp.an_letivo = (SELECT CAST(VALOR as int) FROM _PARAMETROS WHERE CHAVE = 'ANO_BASE')
       ...
       and  ((crp.cur_id = 224 and crp.crp_ordem >= 6 and tmp.cd_escola not in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id = 224 and crp.crp_ordem < 6)
			  or (crp.cur_id = 217 and tmp.cd_escola not in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or crp.cur_id not in (217,219,224,227,228)
			)
    ...
    SELECT tur.tur_id, cur.cur_id, crr.crr_id, crp.crp_id, 1 AS tcr_prioridade, 1 AS tcr_situacao
     ...
     WHERE tmp.an_letivo = (SELECT CAST(VALOR as int) FROM _PARAMETROS WHERE CHAVE = 'ANO_BASE')
       and tmp.cd_tipo_turma <> 3
       and DEPARA_SERIE.cur_id in (SELECT cur_id FROM DEPARA_CURSOS_DIVIDIDOS)
	   and  ((crp.cur_id = 224 and crp.crp_ordem >= 6 and tmp.cd_escola in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id = 219 and tmp.cd_escola in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id in (219,227,228))
			)
    ...
    SELECT tur.tur_id, cur.cur_id, crr.crr_id, crp.crp_id, 1 AS tcr_prioridade, 1 AS tcr_situacao
     ...
     WHERE tmp.an_letivo = (SELECT CAST(VALOR as int) FROM _PARAMETROS WHERE CHAVE = 'ANO_BASE')
       ...
       AND  ((crp.cur_id = 224 and crp.crp_ordem >= 6 and tmp.cd_escola not in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id = 224 and crp.crp_ordem < 6)
			  or (crp.cur_id = 217 and tmp.cd_escola not in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id NOT IN (217,219,224))
			 )
    ...
    SELECT tur.tur_id, cur.cur_id, crr.crr_id, crp.crp_id, 1 AS tcr_prioridade, 1 AS tcr_situacao
     ...
     WHERE tmp.an_letivo = (SELECT CAST(VALOR as int) FROM _PARAMETROS WHERE CHAVE = 'ANO_BASE')
       AND tmp_turma_grade_mstech.dt_fim IS NULL
       and  ((crp.cur_id = 224 and crp.crp_ordem >= 6 and tmp.cd_escola in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id = 217 and tmp.cd_escola in (select esc_codigo from _PARAMETROS_ESCOLA_EDUCACAO_FISICA))
			  or (crp.cur_id in (219,227,228))
			)
	...
	update tur set tur_codigo = tm.dc_turma_escola, tur_dataAlteracao = GETDATE()
	 ...
	 --porém para o Ensino Infantil isso faz sentido, principalmente porque novas escolas e turmas aparecem a qualquer momento e as grades são menos complexas
		...
		select distinct cur_id, crr_id, tur.esc_id, uni_id, 1 as ces_id, '2025-01-01' as ces_vigenciaInicio,
		  ...
		  from GE_TUR_Turma tur
			   ...
			   on tur.tur_id = tcr.tur_id and tcr.cur_id in (213,214,215,216)
			   inner join GE_ESC_Escola esc
			   on tur.esc_id = esc.esc_id
...
BEGIN	
    ...
    SELECT DEPARA_TURMA.tur_id, dis.dis_id, 1 AS tud_verificador, tmpt.tur_codigo, dis.dis_nome AS tud_nome,
           ...
           tud_naoLancarFrequencia = case when dis.tds_id in (11,12) and crd.cur_id = 224 then 1 
										  when dis.tds_id = 10 and crd.crp_id < 6 then 1 
										  when dis.tds_id in (13,30,32) then 1 
										  else 0 end,
           tud_naoExibirFrequencia = case when dis.tds_id in (11,12) and crd.cur_id = 224 then 1 
										  when dis.tds_id = 10 and crd.crp_id < 6 then 1 
										  when dis.tds_id in (13,30,32) then 1 
										  else 0 end,
     ...
     WHERE dct.cd_tipo_turma not in (2, 3)
       ...
       AND crd_situacao = 1
       AND dis_situacao = 1
       AND (crd.cur_id not in (217,224)
           or (crd.cur_id in (217,224) and dis.dis_cargaHorariaTeorica <> 0))
    ...
    SELECT DEPARA_TURMA.tur_id, dis.dis_id, 1 AS tud_verificador, tmpt.tur_codigo, dis.dis_nome AS tud_nome,
      ...
      FROM #RelacaoTurma tmpt inner join tmp_DiarioClasse_turma dct
           ...
           (select tur_id, cur_id, crr_id, crp_id,
                   ROW_NUMBER() OVER (PARTITION BY tur_id ORDER BY CASE WHEN cur_id in (39,40,41,42,227,228,229) 
...
--                        where cur_nome not like '%EJA%' and cur_situacao <> 3)
    ...
    *************************************************/
	...
	select tudExp.tud_id as tud_idExperiencia, tudTer.tud_id as tud_idTerritorio, 
		   GETDATE() as tte_vigenciaInicio, '2024-12-20' as tte_vigenciaFim, 
...
BEGIN
	...
	SELECT @gru_idResponsavel = gru_idUsadoIntegracao FROM DEPARA_GRUPOS_INTEGRACAO 
     WHERE nomeUsadoIntegracao = 'Responsável' and sis_id = 196
       ...
       SELECT usu.usu_id, @gru_idAlunoSerap AS gru_id, 1
         FROM IMP_alunos_pais
              INNER JOIN GE_ACA_AlunoCurriculo alc with (nolock)
               on IMP_alunos_pais.alu_codigo = alc.alc_matricula
              and 3 <> alc.alc_situacao
			  and cur_id not in (213,214,215,216) -- ensino infantil
              INNER JOIN GE_ACA_Aluno alu with (nolock)
              on alc.alu_id = alu.alu_id
              inner join SSO_PES_Pessoa pes with (nolock)
              on alu.pes_id = pes.pes_id
